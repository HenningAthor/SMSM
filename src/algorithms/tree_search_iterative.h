#ifndef SMSM_TREE_SEARCH_ITERATIVE_H
#define SMSM_TREE_SEARCH_ITERATIVE_H
...
public:
    ...
    // holds object for recursive call
    ...
    TreeSearchIterative(T &t,
        ...
        c_managers[0].fill_candidates(n);
        ...
        if (ac.REC_enabled) {
            rec_ac = ac;
            rec_ac.capture_statistics = false;
    ...
    std::vector<uint32_t> search() {
        ...
        if (r == 1) {
            ...
            // search iterativ through the candidates
            iterative_search();
        }
    ...
    void iterative_search() {
        ...
        while (depth != 0 || depth_action != DEPTH_UP) {
            ...
            if (depth_action == DEPTH_STAY) {
                ...
                scores[depth + 1] = c_managers[depth].get_acc(c_managers[depth].offset) ? scores[depth] + c_managers[depth].get_si(c_managers[depth].offset) : sf_1D(s, s_size + 1);
    ...
    */
    void one_element_remaining(const size_t depth) {
        ...
        for (size_t i = p_offset; i < p_size; ++i) {
            if (pc_manager.get_si(i) >= remaining_score) {
                ...
                TypeSF new_score = sf_1D(s, s_size + 1);
    ...
    void one_candidate_too_much(const size_t depth) {
        ...
        TypeSF new_score = sf(s, s_size + r);
        if (new_score > best_score) {
            update_best(s, new_score);
        }
    ...
    /**
     * A wrapper function for the score function.
     *
     * @param temp_s The current set S.
     * @param temp_s_size The current size of S.
     * @param heuristic_id Id of the heuristic making the call (profiling).
     * @param depth The depth at which the function is evaluated (profiling).
     * @param dimension Dimension of the set (used for profiling)
     * @return The score of S.
     */
    TypeSF sf(const std::vector<uint32_t> &temp_s, const size_t temp_s_size) {
        ASSERT(temp_s_size <= k);
        return t.evaluate_general(temp_s, temp_s_size);
    };
     ...
     * @return The score of S.
     */
    TypeSF sf_1D(const std::vector<uint32_t> &temp_s, const size_t temp_s_size) {
        ASSERT(temp_s_size <= k);
        return t.evaluate_1D(temp_s, temp_s_size);
    };
     ...
     * @return The score of S.
     */
    TypeSF sf_2D(const std::vector<uint32_t> &temp_s, const size_t temp_s_size) {
        ASSERT(temp_s_size <= k);
        return t.evaluate_2D(temp_s, temp_s_size);
    };
     ...
     * @return The score of S.
     */
    TypeSF sf_XD(const std::vector<uint32_t> &temp_s, const size_t temp_s_size) {
        ASSERT(temp_s_size <= k);
        return t.evaluate_XD(temp_s, temp_s_size);
    };
    ...
    size_t DCO_depth_0() {
        ...
        for (size_t i = c_manger.offset; i < c_manger.size; ++i) {
            if (c_manger.get_acc(i) == 0) {
                ...
                s[s_size] = c;
                TypeSF si = sf_1D(s, s_size + 1) - score;
    ...
    TypeSF UB2D(const size_t depth) {
        ...
        for (size_t i = 0; i < l; ++i) {
            ...
            for (size_t j = i + 1; j < l; ++j) {
                ...
                if (score_imp < 0) {
                    ...
                    score_imp = sf_2D(s, s_size + 2) - score;
        ...
        for (size_t i = l + r - std::min(n_remaining, l + r); i <= std::min(l, r); ++i) {
            ...
            TypeSF sum_even = std::numeric_limits<TypeSF>::max();
            TypeSF sum_p1 = std::numeric_limits<TypeSF>::max();
            TypeSF sum_p2 = std::numeric_limits<TypeSF>::max();
        ...
        for (size_t b_id = 0; b_id < n_b; ++b_id) {
            ...
            for (size_t j = 3; j <= l_b; ++j) {
                ...
                while (next_subset(comb, j, l_b)) {
                    ...
                    if (score_imp < 0) {
                        score_imp = sf_XD(s, s_size + j) - score;
    ...
    TypeSF REC(const size_t depth) {
        ...
        for (size_t i = 0; i <= r; ++i) {
            ...
            if (n_candidates_p1 == 0) {
                ...
                rec_ac.finalize();
                auto tree_search = TreeSearchIterative<T, TypeSF>(t, rec_ac);
...
#endif //SMSM_TREE_SEARCH_ITERATIVE_H
