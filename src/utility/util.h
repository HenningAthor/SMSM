 ...
 * is already a multiple of `m`, it returns `n` unchanged. Otherwise, it returns the smallest
 ...
 *
 * @param percentage Percentage of completion (between 0.0 and 1.0).
 */
...
/**
 * Generates all random subsets for a clique if we want to choose a matching
 * with k edges.
 *
 * @param max_n Maximum size of the clique.
 */
...
/* Aligned Array Functions */
template<typename T>
void min_64(T *__restrict__ res, T *__restrict__ v1, T *__restrict__ v2, size_t n) {
    res = static_cast<T *>(__builtin_assume_aligned(res, 64));
    v1 = static_cast<T *>(__builtin_assume_aligned(v1, 64));
    v2 = static_cast<T *>(__builtin_assume_aligned(v2, 64));
...
/**
 * Find the minimum value element-wise between two aligned memory blocks and store the result in the first block.
 *
 * This function finds the minimum value element-wise between two aligned memory blocks (res and v2)
 * and stores the result in the first memory block (res). Both memory blocks are assumed to be aligned
 * to a 64-byte boundary, and they each contain 'n' elements.
 *
 * @tparam T The type of elements in the memory blocks.
 * @param res A pointer to the destination aligned memory block where the minimum values will be stored.
 * @param v2 A pointer to the source aligned memory block for comparison.
 * @param n The number of elements to consider in each memory block.
 */
template<typename T>
void min_in_place_64(T *__restrict__ res, const T *__restrict__ v2, size_t n) {
    res = static_cast<T *>(__builtin_assume_aligned(res, 64));
    v2 = static_cast<T *>(__builtin_assume_aligned(v2, 64));
    if (n & 63) __builtin_unreachable();
...
/**
 * Calculate the sum of minimum values from two aligned memory blocks.
 *
 * This function computes the sum of the minimum values from two aligned memory blocks
 * (v1 and v2) where each block contains elements of type T. It assumes that both memory
 * blocks are aligned to a 64-byte boundary and that they each contain 'n' elements.
 *
 * @tparam T The type of elements in the memory blocks.
 * @param v1 A pointer to the first aligned memory block for comparison.
 * @param v2 A pointer to the second aligned memory block for comparison.
 * @param n The number of elements to consider in each memory block.
 * @return The sum of the minimum values from the two memory blocks.
 */
template<typename T>
T sum_of_min_64(T *__restrict__ v1, T *__restrict__ v2, size_t n) {
    v1 = static_cast<T *>(__builtin_assume_aligned(v1, 64));
    v2 = static_cast<T *>(__builtin_assume_aligned(v2, 64));
    if (n & 63) __builtin_unreachable();
...
/**
 * Calculate the sum of minimum values from three aligned memory blocks.
 *
 * This function computes the sum of the minimum values from three aligned memory blocks
 * (v1, v2, and v3) where each block contains elements of type T. It assumes that all three
 * memory blocks are aligned to a 64-byte boundary and that they each contain 'n' elements.
 *
 * @tparam T The type of elements in the memory blocks.
 * @param v1 A pointer to the first aligned memory block for comparison.
 * @param v2 A pointer to the second aligned memory block for comparison.
 * @param v3 A pointer to the third aligned memory block for comparison.
 * @param n The number of elements to consider in each memory block.
 * @return The sum of the minimum values from the three memory blocks.
 */
template<typename T>
T sum_of_min_64(T *__restrict__ v1, T *__restrict__ v2, T *__restrict__ v3, size_t n) {
    v1 = static_cast<T *>(__builtin_assume_aligned(v1, 64));
    v2 = static_cast<T *>(__builtin_assume_aligned(v2, 64));
    v3 = static_cast<T *>(__builtin_assume_aligned(v3, 64));
    if (n & 63) __builtin_unreachable();
...
/**
 * Overwrite the contents of one aligned memory block with another.
 *
 * @tparam T The type of elements in the memory blocks.
 * @param v1 A pointer to the destination aligned memory block to overwrite.
 * @param v2 A pointer to the source aligned memory block whose elements will be copied to v1.
 * @param n The number of elements to copy.
 */
template<typename T>
void overwrite_64(T *__restrict__ v1, T *__restrict__ v2, size_t n) {
    v1 = static_cast<T *>(__builtin_assume_aligned(v1, 64));
    v2 = static_cast<T *>(__builtin_assume_aligned(v2, 64));
    if (n & 63) __builtin_unreachable();
...
/**
 * Calculate the sum of elements in an aligned vector.
 *
 * @tparam T The type of elements in the vector.
 * @param vec The aligned vector for which to calculate the sum.
 * @return The sum of all elements in the vector.
 */
template<typename T>
T sum(std::vector<T, boost::alignment::aligned_allocator<T, 64>> &vec) {
    T s = 0;
    for (size_t i = 0; i < vec.size(); ++i) {
        s += vec[i];
    }
...
/**
 * Find the minimum values between two aligned vectors and store the result in a third vector.
 *
 * @tparam T The type of elements in the vectors.
 * @param res The destination aligned vector to store the minimum values.
 * @param v1 The first source aligned vector for comparison.
 * @param v2 The second source aligned vector for comparison.
 */
template<typename T>
void min(std::vector<T, boost::alignment::aligned_allocator<T, 64>> &res,
         std::vector<T, boost::alignment::aligned_allocator<T, 64>> &v1,
         std::vector<T, boost::alignment::aligned_allocator<T, 64>> &v2) {
    min_64(res.data(), v1.data(), v2.data(), res.size());
}

/**
 * Find the minimum values between two aligned vectors and store the result in the first vector.
 *
 * @tparam T The type of elements in the vectors.
 * @param res The destination aligned vector to store the minimum values.
 * @param v2 The source aligned vector to compare with.
 */
template<typename T>
void min_in_place(std::vector<T, boost::alignment::aligned_allocator<T, 64>> &res,
                  std::vector<T, boost::alignment::aligned_allocator<T, 64>> &v2) {
    min_in_place_64(res.data(), v2.data(), res.size());
}

/**
 * Calculate the sum of minimum values from two aligned vectors.
 *
 * This function computes the sum of the minimum values from two aligned vectors.
 * It assumes that both vectors have the same size.
 *
 * @tparam T The type of elements in the vectors.
 * @param v1 The first aligned vector.
 * @param v2 The second aligned vector.
 * @return The sum of the minimum values from the two vectors.
 */
template<typename T>
T sum_of_min(std::vector<T, boost::alignment::aligned_allocator<T, 64>> &v1,
             std::vector<T, boost::alignment::aligned_allocator<T, 64>> &v2) {
    return sum_of_min_64(v1.data(), v2.data(), v1.size());
}

/**
 * Calculate the sum of minimum values from three aligned vectors.
 *
 * @tparam T The type of elements in the vectors.
 * @param v1 The first aligned vector.
 * @param v2 The second aligned vector.
 * @param v3 The third aligned vector.
 * @return The sum of the minimum values from the three vectors.
 */
template<typename T>
T sum_of_min(std::vector<T, boost::alignment::aligned_allocator<T, 64>> &v1,
             std::vector<T, boost::alignment::aligned_allocator<T, 64>> &v2,
             std::vector<T, boost::alignment::aligned_allocator<T, 64>> &v3) {
    return sum_of_min_64(v1.data(), v2.data(), v3.data(), v1.size());
}

/**
 * Overwrite the contents of one aligned vector with another.
 *
 * @tparam T The type of elements in the vectors.
 * @param v1 The destination aligned vector to overwrite.
 * @param v2 The source aligned vector whose elements will be copied to v1.
 */
template<typename T>
void overwrite(std::vector<T, boost::alignment::aligned_allocator<T, 64>> &v1,
               std::vector<T, boost::alignment::aligned_allocator<T, 64>> &v2) {
    overwrite_64(v1.data(), v2.data(), v1.size());
}
